...
public class SbJpaCrudManyToManyApplication implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		...
		repository.addStudent(studentOne);
		repository.addStudent(studentTwo);
		
		repository.findAllCoursesByStudent(1).forEach(corsi -> System.out.println(corsi.toString()));
        repository.findAllStudentByCourse(2).forEach(studenti -> System.out.println(studenti.toString()));
